...
class JiraConnector(object):
    ...
    def md_text_to_jira_text_field(self, md_text):
        ...
        text = markdown_comment_pattern.sub(
            lambda m: f"[{m.group(1)}|{self.jira.server_url}/browse/{m.group(2)}?focusedId={m.group(3)}&page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel#comment-{m.group(3)}]",
            md_text
        )
